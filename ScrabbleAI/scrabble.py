...
#local files
import board, tile, bag, player, human, ai, heuristic, myai
...
def runGame(USERDATA, useHintBox = False):	
	...
	#players.append(myai.AI(theBoard, theBag))
	players.append(human.Human("Player", theBoard, theBag))
	active = 0
	
	computerTurn = isinstance(players[active], ai.AI)or isinstance(players[active], myai.AI)
	firstTurn = True
	gameOver = False
	...
	if not isPractice:
		...
		for p in players:
			if isinstance(p, human.Human):
				if userdata.__contains__("bestScore") and p.score > userdata["bestScore"]:
		...
		if isinstance(maxPlayer, human.Human):
			if userdata.__contains__("numVictories"):
...
def saveUser(USERDATA):
	...
	if USERDATA.__contains__("bestScore"):
	...
	if USERDATA.__contains__("numGames"):
